...
parsing_rules = False
rules = []
current_rule = {'index': None, 'action': None, 'src': [], 'dst': [], 'install': [], 'services': []}
...
def p_obj_name_line(p):
    ...
    if cptr == 2 : current_obj['name'] = p[3]
    cptr += 1

def p_obj_name_line1(p):
    '''obj_name_line : COLON LPAREN NUMBER WORD'''
    global cptr, current_obj
    if cptr == 2 : current_obj['name'] = p[3] + p[4]
    cptr += 1
...
def p_type_line(p):
    ...
    global cptr, current_obj, current_iface, parsing_group_attr, parsing_object_type, current_rule, parsing_exception
    if cptr == 3 :
...
def p_interface_begin(p):
    ...
    global cptr, current_obj, current_fw, current_iface
...
def p_service_port(p):
    '''service_port : COLONPORT LPAREN NUMBER RPAREN'''
    global parsing_object_type
    current_obj['port'] = p[3]

### Parsing a port range.

def p_service_port2(p):
    '''service_port : COLONPORT LPAREN NUMBER WORD RPAREN'''
    global parsing_object_type
    if parsing_object_type == 'services':
        current_obj['portL'] = p[3]
        current_obj['portR'] = p[4][1:]

def p_service_port3(p):
    '''service_port : COLONPORT LPAREN WORD RPAREN'''
    global parsing_object_type
    if parsing_object_type == 'services':
        s = p[3].replace("\"", "")
        signe = s[0]
...
def p_service_protocol(p):
    '''service_port : COLONPROTO LPAREN NUMBER RPAREN
                    | COLONPROTO LPAREN WORD RPAREN'''
    global parsing_object_type
    if parsing_object_type == 'services':
        current_obj['protocol'] = p[3]


##### Parsing rules ####

def p_begin_rule_line(p):
    '''begin_rule_line : COLON RULES LPAREN'''
...
def p_rule_number_line(p):
    '''rule_number_line : COLON LPAREN RULE'''
    global cptr, current_rule
    cptr  = 2
    if cptr == 2:
        current_rule = {'index': p[3][5:], 'action': None, 'src': [], 'dst': [], 'install': [], 'services': []}
    cptr += 1
...
def p_begin_services(p):
    '''begin_services : COLON SERVICES LPAREN'''
    global cptr, rule_attr
    cptr += 1
    rule_attr = "services"
...
def p_attr_line(p):
    '''attr_line : COLON LPAREN WORD RPAREN
                 | COLON LPAREN RPAREN
                 | COLON WORD LPAREN NUMBER RPAREN
                 | COLON LPAREN ANY RPAREN
                 | COLON WORD LPAREN ANY RPAREN
                 ...
                 | COLONPORT WORD LPAREN WORD RPAREN
                 | COLONPORT WORD LPAREN NUMBER RPAREN
                 | COLONPORT WORD LPAREN RPAREN
                 | COLONPROTO WORD LPAREN RPAREN
                 | COLONPROTO WORD LPAREN WORD RPAREN
                 | COLONPROTO WORD LPAREN NUMBER RPAREN
                 |
                 '''
    pass
...
def p_end_obj(p):
    ...
    global cptr, current_obj, object_dict, current_fw, firewalls, parsing_group_attr, \
        parsing_object_type, current_rule, rules, services, j
    cptr -= 1
    if cptr == 2:
        try :
